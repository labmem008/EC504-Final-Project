...
Consider a database matrix **D** with `n` vectors of size `d`. This database **D** can be projected onto a lower dimensional space with `n` vectors of size `k` using a random projection matrix.
